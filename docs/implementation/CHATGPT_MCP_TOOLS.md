...
| Timeline 实体同时有 `description` 与 `content` | 格式化层将 `description` 构造为 `content` 的展示别名(恒等值,兼容原 Web UI),非数据库泄漏 | **已在 MCP 输出层清理**(2026-09-14 最终裁决§三/§六):`toMcpTimelineEvent` 适配器使 MCP 全部 timeline 输出(create/update/get_story_item/list_trash/restore_item/restore_item_version)仅含 canonical `content`,不再返回 `description`;REST / Web / 共享 formatter / 数据库行为不变,测试锁定 |
...
命名风格最终裁决(2026-09-14):完整实体 = camelCase 契约,context/索引 = compact snake_case 形状,同一对象内不得出现双份字段;`expected_updated_at` 取完整实体返回的 `updatedAt` 值。全局统一命名明确留给 V2 单独的兼容性升级。

V2 cleanup 候选清单(均为兼容性评估后再动的非阻塞项):统一实体/索引两种命名风格。
